...
module.exports = (requireAuth) => {
    router.get('/', requireAuth, requireRole('admin'), async (req, res) => {
        try {
            ...
            logger.error({ err: e }, 'Users route Fehler');
    ...
    router.post('/', requireAuth, requireRole('admin'), validate(userSchema), async (req, res) => {
        try {
            ...
            if (existing)
                return res
                    .status(400)
                    .json({ success: false, reason: 'Benutzername existiert bereits.' });
            const plainPass = crypto.randomBytes(12).toString('hex');
            u.pass = await bcrypt.hash(plainPass, 12);
            u.require_password_change = 1;
            await DB.addUser(u);
            if (u.email)
                Mailer.sendUserCredentials(u.email, u.name, u.user, plainPass, DB).catch((e) =>
                    logger.error({ err: e }, 'Mailer Fehler beim User-Anlegen')
                );
            try {
                if (DB.addAuditLog)
                ...
                try {
                    if (DB.addAuditLog)
                        await DB.addAuditLog({
                            ...
                            action: 'user.update',
                            entity: 'user',
                            entity_id: req.params.user,
                            detail: { role: safeUpdate.role },
                        });
                } catch (_) {}
                res.json({ success: true });
            } catch (e) {
    ...
    router.delete('/:user', requireAuth, requireRole('admin'), async (req, res) => {
        try {
            if (req.params.user === req.admin.user)
                return res
                    .status(400)
                    .json({ success: false, reason: 'Kann sich selbst nicht löschen.' });
        ...
        async (req, res) => {
            try {
                ...
                if (!target)
                    return res
                        .status(404)
                        .json({ success: false, reason: 'Benutzer nicht gefunden.' });
                if (!target.email)
                    return res.status(400).json({
                        success: false,
                        reason: 'Benutzer hat keine E-Mail Adresse hinterlegt.',
                    });
                const plainPass = crypto.randomBytes(12).toString('hex');
                const hashed = await bcrypt.hash(plainPass, 12);
                await DB.setUserPass(target.user, hashed, true);
                Mailer.sendUserCredentials(
                    target.email,
                    target.name,
                    target.user,
                    plainPass,
                    DB
                ).catch((e) => logger.error({ err: e }, 'Mailer Fehler beim Passwort-Reset'));
                try {
                    if (DB.addAuditLog)
                        await DB.addAuditLog({
                            actor: req.admin?.user || null,
