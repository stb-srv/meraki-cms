    ...
    if (openMin !== null && closeMin !== null) {
        if (nowMin < openMin) {
            return {
                ...
                openMin,
                closeMin,
                openStr: dayConfig.open,
                closeStr: dayConfig.close,
                cutoff,
                lead,
            };
        }
...
function validatePickupTime(pickupTime, openStatus) {
    ...
    if (openStatus.openMin !== null && pickupMin < openStatus.openMin) {
        return {
            valid: false,
            reason: `Abholzeit liegt vor der Öffnungszeit (${openStatus.openStr} Uhr).`,
        };
    }
    ...
    router.get('/config', async (req, res) => {
        try {
            ...
            res.json({
                ...
                deliveryEnabled:
                    onlineOrdersEnabled &&
                    orderConfig.ordersEnabled === true &&
                    orderConfig.deliveryEnabled === true,
                pickupEnabled:
                    onlineOrdersEnabled &&
                    orderConfig.ordersEnabled === true &&
                    orderConfig.pickupEnabled === true,
                dineInEnabled:
                    onlineOrdersEnabled &&
                    orderConfig.ordersEnabled === true &&
                    orderConfig.dineInEnabled === true,
        ...
        async (req, res) => {
            try {
                ...
                if (!type || !['dine_in', 'pickup', 'delivery'].includes(type)) {
                    return res
                        .status(400)
                        .json({ success: false, reason: 'Ungültiger Bestelltyp.' });
                }
                ...
                if (items.length > MAX_ITEMS_PER_ORDER) {
                    return res.status(400).json({
                        success: false,
                        reason: `Maximale Artikelanzahl (${MAX_ITEMS_PER_ORDER}) überschritten.`,
                    });
                }
                ...
                if (type !== 'delivery' && !cleanPhone) {
                    return res.status(400).json({
                        success: false,
                        reason: 'Bitte geben Sie eine Telefonnummer für Rückfragen an.',
                    });
                }
                ...
                if (
                    ...
                    return res.status(400).json({
                        success: false,
                        reason: 'Bitte gib eine E-Mail-Adresse an (für die Bestellbestätigung).',
                    });
                }
                ...
                if (!orderConfig.ordersEnabled)
                    return res
                        .status(403)
                        .json({ success: false, reason: 'Bestellsystem ist derzeit deaktiviert.' });
                if (type === 'delivery' && !orderConfig.deliveryEnabled)
                    return res
                        .status(403)
                        .json({ success: false, reason: 'Lieferung ist derzeit deaktiviert.' });
                if (type === 'pickup' && !orderConfig.pickupEnabled)
                    return res
                        .status(403)
                        .json({ success: false, reason: 'Abholung ist derzeit deaktiviert.' });
                if (type === 'dine_in' && !orderConfig.dineInEnabled)
                    return res.status(403).json({
                        success: false,
                        reason: 'Tisch-Bestellung ist derzeit deaktiviert.',
                    });
                ...
                for (const item of items) {
                    ...
                    if (!dbItem)
                        return res
                            .status(400)
                            .json({ success: false, reason: `Unbekanntes Gericht: ${item.id}` });
                    if (dbItem.available === false)
                        return res.status(400).json({
                            success: false,
                            reason: `Gericht nicht verfügbar: ${dbItem.name}`,
                        });
                    const qty = Math.max(
                        1,
                        Math.min(MAX_QTY_PER_ITEM, parseInt(item.quantity, 10) || 1)
                    );
                    validatedItems.push({
                        id: dbItem.id,
                        name: dbItem.name,
                ...
                const order = {
                    ...
                    deliveryAddress:
                        type === 'delivery'
                            ? sanitizeText(deliveryAddress || delivery?.address || '').slice(
                                  0,
                                  300
                              ) || null
                            : null,
