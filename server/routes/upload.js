    ...
    const upload = multer({
        ...
        fileFilter: (req, file, cb) => {
            ...
            if (mimeOk && extOk) {
                ...
                const err = new Error(
                    'Nur Bilddateien erlaubt (jpg, jpeg, png, gif, webp). SVG ist aus Sicherheitsgründen nicht erlaubt.'
                );
        ...
        upload.single('image')(req, res, async (err) => {
            ...
            try {
                ...
                if (!type || !ALLOWED_MIMETYPES.includes(type.mime)) {
                    ...
                    return res
                        .status(400)
                        .json({ success: false, reason: 'Ungültiger Dateityp (Magic Bytes).' });
                }
            } catch (checkErr) {
                if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
                return res
                    .status(500)
                    .json({ success: false, reason: 'Fehler bei der Dateiprüfung.' });
            }
