...
window.MenuTranslate = {
        ...
        if (exportTrBtn)
            exportTrBtn.onclick = async () => {
                try {
                    ...
                    window.MenuCore.utils.showToast('Export fehlgeschlagen', 'error');
                }
            };
                        ...
                        try {
                            ...
                            if (
                                await window.MenuCore.utils.showConfirm(
                                    ...
                                    `${dishCount} Gerichte in der Datei gefunden. Vorhandene Übersetzungen werden ergänzt.`
                                )
                            ) {
                                const res = await window.MenuCore.api.post(
                            ...
                            window.MenuCore.utils.showToast('Fehler: ' + err.message, 'error');
                        }
                    };
    ...
    openAiImageModal: async function () {
        ...
        if (!modal) {
            ...
            modal.style.cssText =
                'display:none; position:fixed; inset:0; background:rgba(0,0,0,0.6); z-index:11000; align-items:center; justify-content:center; backdrop-filter:blur(4px);';
            modal.innerHTML = `
        ...
        searchBtn.onclick = async () => {
            ...
            resultsGrid.innerHTML =
                '<p style="grid-column:1/-1; text-align:center; padding:40px;">Bilder werden geladen...</p>';
            try {
                const res = await window.MenuCore.api.post('image-ai/search', {
                    query,
                    provider: 'unsplash',
                });
                if (res && res.success) {
                    ...
                    res.results.forEach((img) => {
                        ...
                        div.style.cssText =
                            'cursor:pointer; border-radius:12px; overflow:hidden; aspect-ratio:1/1; border:3px solid transparent;';
                        ...
                        div.onclick = () => {
                            ...
                            document.getElementById('df-img-preview').innerHTML =
                                `<img src="${img.url}" style="width:100%; height:100%; object-fit:cover; border-radius:10px;">`;
                ...
                resultsGrid.innerHTML = '<p>Fehler</p>';
            }
        };
    },
};
