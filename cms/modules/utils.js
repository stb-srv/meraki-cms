...
export const showToast = (message, type = 'success') => {
    ...
    d.style.cssText = `
        ...
        background: ${type === 'success' ? '#16a34a' : type === 'warning' ? '#d97706' : '#dc2626'};
        color: #fff;
        padding: 14px 36px;
        border-radius: 12px;
        z-index: 10000;
...
export const showPrompt = (title, text) => {
    return new Promise((resolve) => {
        ...
        inp.onkeydown = (e) => {
            if (e.key === 'Enter') document.getElementById('mp-ok').click();
        };
    });
};
...
export const showSelect = (title, text, options = []) => {
    return new Promise((resolve) => {
        ...
        document.getElementById('ms-ok').onclick = () => {
            const v = document.getElementById('ms-input').value;
            div.remove();
            resolve(v);
        };
    });
...
export const showHelp = (topic) => {
    ...
    if (target.sections) {
        const sectionsHtml = target.sections
            .map(
                (s) => `
