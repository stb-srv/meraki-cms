document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function checkLiveAvailability() {
        ...
        if (!oh || oh.closed) {
            document.getElementById('res-time-grid').innerHTML =
                '<p style="text-align:center; padding:40px; color:#ef4444;">Wir haben an diesem Tag Ruhetag.</p>';
            return;
        }
        ...
        while (curr <= endD) {
            times.push(
                curr.getHours().toString().padStart(2, '0') +
                    ':' +
                    curr.getMinutes().toString().padStart(2, '0')
            );
    ...
    async function get(r) {
        try {
            return await (await fetch(`${API}/${r}`)).json();
        } catch {
            return null;
        }
    }
    ...
    let init = async function init() {
        ...
        if (hp) {
            ...
            if (hp.activeModules?.reservations === false) {
                ...
                if (resV) {
                    ...
                    const phoneSuffix = br?.phone
                        ? `<br><br><a href="tel:${br.phone}" class="btn-premium" style="display:inline-block; text-decoration:none; margin-top:10px;"><i class="fas fa-phone-alt"></i> ${br.phone}</a>`
                        : '';
                    resV.innerHTML = `
        ...
        if (m && Array.isArray(m)) {
            ...
            if (list)
                list.innerHTML =
                    '<p style="text-align:center;padding:40px;opacity:.5;">Speisekarte konnte nicht geladen werden.</p>';
        }
        ...
        if (resForm) {
            resForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const rawTime =
                    document.getElementById('res-time')?.value.replace(' Uhr', '') || '';
                const data = {
                ...
                try {
                    ...
                    toast(OpaI18n.t('reservations.error'));
                }
            });
        }
    ...
    function applyBranding(d) {
        ...
        if (d.bgImage) {
            ...
            document.getElementById('hero-bg').style.backgroundImage =
                `url('/admin/assets/santorini_bg.png')`;
        }
        ...
        if (d.promotionText && d.promotionEnabled !== false)
            document.getElementById('promo-text').textContent = d.promotionText;
        ...
        if (d.openingHours) {
            ...
            if (rows) {
                rows.innerHTML = days
                    .map((day) => {
                        ...
                        const dayLabel = window.OpaI18n
                            ? OpaI18n.t(`opening_hours.days.${day}`)
                            : day;
                        const ruhetag = window.OpaI18n
                            ? OpaI18n.t('opening_hours.ruhetag')
                            : 'Ruhetag';
                        const uhr = window.OpaI18n ? OpaI18n.t('opening_hours.uhr') : 'Uhr';
                        const todayLbl = window.OpaI18n
                            ? OpaI18n.t('opening_hours.today')
                            : 'Heute';

                        const timeStr = entry.closed
                            ? ruhetag
                            : `${entry.open} – ${entry.close} ${uhr}`.trim();
                        const todayBadge = isToday
                            ? ` <span style="font-size:.65rem; background:var(--gold,#C8A96E); color:#fff; padding:1px 6px; border-radius:10px; margin-left:4px;">${todayLbl}</span>`
                            : '';
            ...
            if (badge) {
                ...
                if (!todayEntry || todayEntry.closed) {
                    badge.textContent = window.OpaI18n
                        ? OpaI18n.t('opening_hours.closed_today')
                        : 'Heute geschlossen';
                    ...
                    if (nowMins >= openMins && nowMins < closeMins) {
                        if (closeMins - nowMins <= 60) {
                            badge.textContent = OpaI18n
                                ? OpaI18n.t('opening_hours.closes_soon', { time: todayEntry.close })
                                : `Schließt ${todayEntry.close} Uhr`;
                            badge.style.background = '#f59e0b';
                        } else {
                            badge.textContent = OpaI18n
                                ? OpaI18n.t('opening_hours.open_until', { time: todayEntry.close })
                                : `Geöffnet bis ${todayEntry.close}`;
                        ...
                        badge.textContent =
                            nowMins < openMins
                                ? OpaI18n
                                    ? OpaI18n.t('opening_hours.opens_at', { time: todayEntry.open })
    ...
    function showMenuSkeleton(count = 6) {
        ...
        if (catEl) {
            catEl.innerHTML = Array(5)
                .fill(0)
                .map(() => `<button class="cat-btn cat-btn--skeleton skeleton"></button>`)
                .join('');
        }
    ...
    function renderCategories() {
        ...
        if (window.MERAKI_DAILY_SPECIALS_ENABLED) {
            const hasSpecials = menuItems.some(
                (i) => i.is_daily_special && i.active !== false && i.available !== false
            );
            if (hasSpecials) {
                const label = window.OpaI18n
                    ? OpaI18n.t('menu.daily_specials').replace('⭐ ', '')
                    : 'Tagesspecials';
    ...
    function updateFavCatBtn() {
        ...
        if (favSet.size > 0 && !favBtn) {
            ...
            const label = window.OpaI18n
                ? OpaI18n.t('menu.favorites').replace('❤️ ', '')
                : 'Favoriten';
            favBtn.innerHTML = `❤️ ${label} (${favSet.size})`;
            favBtn.onclick = function () {
                window.filterMenu('__fav__', this);
            };
    ...
    function applyMenuFilter() {
        ...
        let items = menuItems.filter(
            (i) => i.active !== false && i.available !== false && isAvailableToday(i)
        );
    ...
    window.switchTab = (id) => {
        ...
        window.scrollTo({
            top: id === 'home' ? 0 : document.getElementById('main-nav').offsetHeight,
            behavior: 'smooth',
        });
    };
    ...
    function initPageSliders(container) {
        container.querySelectorAll('.opa-page-slider').forEach((slider) => {
            ...
            if (autoplay)
                slider._opaTimer = setInterval(() => window.opaSlide(slider.id, 1), interval);
        });
    }
    ...
    window.opaSlideGo = (id, idx) => {
        ...
        slider.querySelectorAll('[data-dot]').forEach((dot) => {
            dot.style.background =
                parseInt(dot.dataset.dot) === idx ? 'rgba(255,255,255,1)' : 'rgba(255,255,255,.4)';
        ...
        if (slider._opaTimer) {
            ...
            if (slider.dataset.autoplay === '1')
                slider._opaTimer = setInterval(() => window.opaSlide(id, 1), interval);
        }
    };
    ...
    window.setLegalView = (type) => {
        ...
        if (homeData.legal) {
            if (titleEl)
                titleEl.textContent = type === 'impressum' ? 'Impressum' : 'Datenschutzerklärung';
            if (contentEl)
    ...
    window.navTime = (delta) => {
        ...
        timeOffset = Math.max(
            0,
            Math.min(timeOffset + delta * TIME_PAGE_SIZE, allSlots.length - 1)
        );

        allSlots.forEach((slot, i) => {
            slot.style.display =
                i >= timeOffset && i < timeOffset + TIME_PAGE_SIZE * 3 ? '' : 'none';
        });

        // Scroll zu den sichtbaren Slots
        const firstVisible = document.querySelector(
            '#res-time-grid .time-slot:not([style*="none"])'
        );
    ...
    function renderResCalendar() {
        ...
        let html = ['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So']
            .map((d) => `<div class="cal-modern-head">${d}</div>`)
            .join('');
        ...
        if (areas) {
            ...
            if (list) {
                list.innerHTML = areas
                    .map(
                        (a) =>
                            `<div class="area-tab" data-id="${a.id}" onclick="window.selectArea('${a.id}')">${a.name} <i class="fas fa-chevron-right"></i></div>`
                    )
                    .join('');
    ...
    function checkVacationStatus(v) {
        ...
        if (isActive) {
            let seen = false;
            try {
                seen = sessionStorage.getItem('meraki_vacation_seen');
            } catch (e) {}
            if (!seen) {
    ...
    window.closeVacation = () => {
        ...
        if (modal) {
            ...
            try {
                sessionStorage.setItem('meraki_vacation_seen', '1');
            } catch (e) {}
        }
    };
    ...
    function checkHolidayStatus(v) {
        ...
        let seen = false;
        try {
            seen = sessionStorage.getItem('meraki_holiday_seen');
        } catch (e) {}
        if (start && end && now >= start && now <= end && !seen) {
            const modal = document.getElementById('holiday-modal');
            if (modal) {
                document.getElementById('holiday-title').textContent = v.title || 'Feiertags-Info';
                document.getElementById('holiday-text').textContent =
                    v.text || 'Gern sind wir für Sie da!';
                modal.classList.add('active');
            }
        }
    }
    ...
    window.closeHoliday = () => {
        ...
        if (modal) {
            modal.classList.remove('active');
            try {
                sessionStorage.setItem('meraki_holiday_seen', '1');
            } catch (e) {}
        }
    };
